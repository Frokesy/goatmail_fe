...
const ConversationMode = () => {
  ...
  useEffect(() => {
    (async () => {
      const saved = await AsyncStorage.getItem("conversationMode");
      if (saved !== null) {
  ...
  const toggleSwitch = async () => {
    ...
    await AsyncStorage.setItem("conversationMode", JSON.stringify(newValue));
      ...
      <ScrollView>
        ...
        <View className="px-6 pt-10 pb-4 flex-row items-center justify-between border-b border-[#E4E4E7]">
          <View className="flex-1 pr-4">
            <Text className="text-[16px] font-semibold">Conversation mode</Text>
            <Text className="text-[14px] text-[#737373] mt-2">
              Group mails with the same subject
            </Text>
          </View>
